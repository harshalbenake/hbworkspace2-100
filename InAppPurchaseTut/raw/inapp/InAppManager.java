package com.fastre.inapp;

import com.fastre.R;
import com.fastre.SettingsScreen;
import com.fastre.asynctask.Async_UpgradeToPremimum;
import com.fastre.inapp.utils.IabException;
import com.fastre.inapp.utils.IabHelper;
import com.fastre.inapp.utils.IabHelper.OnConsumeFinishedListener;
import com.fastre.inapp.utils.IabResult;
import com.fastre.inapp.utils.Inventory;
import com.fastre.inapp.utils.Purchase;
import com.fastre.inapp.utils.SkuDetails;
...
public class InAppManager {
	...
	public InAppManager(Activity activity) {
		...
		String base64EncodedPublicKey = mActivity.getString(R.string.base64_encoded_public_key);
		mPayload = mActivity.getResources().getString(R.string.in_app_purchase_payload_key); // secret
																								// key
	...
	boolean verifyDeveloperPayload(Purchase p) {
		...
		/*
		 * TODO: verify that the developer payload of the purchase is correct. It will be the same
		 * one that you sent when initiating the purchase. WARNING: Locally generating a random
		 * string when starting a purchase and verifying it here might seem like a good approach,
		 * but this will fail in the case where the user purchases an item on one device and then
		 * uses your app on a different device, because on the other device you will not have access
		 * to the random string you originally generated. So a good developer payload has these
		 * characteristics: 1. If two different users purchase an item, the payload is different
		 * between them, so that one user's purchase can't be replayed to another user. 2. The
		 * payload must be such that you can verify it even when the app wasn't the one who
		 * initiated the purchase flow (so that items purchased by the user on one device work on
		 * other devices owned by the user). Using your own server to store and verify developer
	...
	public void setUpgradeToPremimumClickListener(View view) {
	...
	void alert(String message) {
		CustomToast.makeText(mActivity, message, CustomToast.LENGTH_LONG).show();
	...
	private void changeUserAccountStatus(Purchase purchase) {
		...
		/**
		 * This will change user account type on FastRe Server.
		 */
		new Async_UpgradeToPremimum(mActivity, purchase, skuDetails) {
			protected void onPreExecute() {
				if (Utility.isConnected(mActivity)) {
					setWaitScreen(true);
				} else {
					CustomToast.makeText(mActivity, mActivity.getString(R.string.check_internet_connection), CustomToast.LENGTH_SHORT).show();
					cancel(true);
				}
			};
			...
			protected void onPostExecute(String result) {
				...
				if (result != null && !result.equalsIgnoreCase("")) {
					AccountStatus accountStatus = new Gson().fromJson(result, AccountStatus.class);
					if (accountStatus.status != null && accountStatus.status.equalsIgnoreCase("success")) {
						PreferencesManagerClass prefManagerClass = new PreferencesManagerClass(mActivity);
						prefManagerClass.storeUserAccountStatus(accountStatus);
						new UserManager().updateUserPremiumUser(mActivity, "P", new UserManager().getUserId(mActivity));
						((SettingsScreen) mActivity).setAccountStatus();

					} else {
						CustomToast.makeText(mActivity, "Error while upgrading to premimum.", CustomToast.LENGTH_SHORT).show();
					}
